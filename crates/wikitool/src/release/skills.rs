...
use crate::skills::{
    SKILLS_MANIFEST_SCHEMA, SkillManifestFile, SkillsManifest, SkillManifestEntry, PUBLIC_SKILL_IDS,
    load_skills, sha256_bytes,
};
...
pub(super) fn run_release_build_skills(args: ReleaseBuildSkillsArgs) -> Result<()> {
    ...
    println!("release build-agent-skills");
...
pub(super) fn print_skills_build(result: &SkillsBuildResult) {
    ...
    println!("skills_skills: {}", result.skill_count);
    println!("skills_files: {}", result.file_count);
}

pub(super) fn build_skills(
    repo_root: &Path,
    output_dir: &Path,
) -> Result<SkillsBuildResult> {
    let source_root = repo_root.join("agent-skills");
    require_regular_file(&source_root.join("README.md"), "Wikitool skills README")?;
    require_directory(
        &source_root.join("integration"),
        "Wikitool skills integration directory",
    )?;
    require_directory(&source_root.join("skills"), "Wikitool skills skills directory")?;

    validate_release_output(repo_root, output_dir, "Wikitool skills output")?;
...
fn copy_regular_tree(source_root: &Path, destination_root: &Path) -> Result<()> {
    for relative in collect_regular_files(source_root)? {
        if relative == Path::new("manifest.json") {
            bail!("Wikitool skills source must not contain a generated manifest.json");
        }
        copy_file(
    ...
    while let Some(directory) = pending.pop() {
        ...
        for entry in entries {
            ...
            if metadata.file_type().is_symlink() {
                bail!(
                    "Wikitool skills source contains a symlink: {}",
                    normalize_path(&path)
                );
            }
            if metadata.is_dir() {
                ...
                bail!(
                    "unsupported Wikitool skills source entry: {}",
                    normalize_path(&path)
                );
            }
        }
...
mod tests {
    ...
    fn source_tree_cannot_be_selected_as_skills_output() {
        let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");
        let error = build_skills(&repo_root, &repo_root.join("agent-skills"))
