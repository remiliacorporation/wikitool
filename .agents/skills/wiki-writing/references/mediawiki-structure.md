...
Every article that uses inline `<ref>` citations must also render them. Use the adapter's declared references template when one is available; otherwise add a `== References ==` section containing `<references />`. Define each named reference at its first use in the body; list-defined references such as `{{Reflist|refs=...}}` need the site's template to declare that parameter, which the local template catalog shows. A clean claim map does not excuse an unrendered citation list.
